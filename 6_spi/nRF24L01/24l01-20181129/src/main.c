...
int main( void )
{
	...
	pinMode( CSPINNUM, OUTPUT );
	digitalWrite( CSPINNUM, HIGH );
    delay( 2 );

	while ( 1 )
	{
		printf(" SPI cs_change Test -----\n");
			
		NRF24L01_WriteRegCsChange( 0, 0xff, 0xff );
		
		//delay(2);
		
		//NRF24L01_WriteRegCsChange( 0, 0x01, 0xFF );
	...
	while ( 1 )
	{
		...
		nTemp = NRF24L01_WriteReg( 0xff, 0xff );
